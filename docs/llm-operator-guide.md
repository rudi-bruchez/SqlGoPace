...
too-broad rule. A shrink that omits the key gets two minutes rather than no cap; every other
operation still gets no cap when it omits it. When the engine reacts to blocking it writes an advisory `<manifest>.blocked.yaml` next
