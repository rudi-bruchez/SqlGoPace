...
6. Task 6 lacks exact insertion points.
   The plan says "add a subsection" in `specs/SHRINK.md` and "one line under the shrink / contended-capture description" in `README.md` without line numbers. The implementer will need to locate the relevant sections. This is reasonable for a plan, but consider adding file-search hints (e.g., grep for `confirmed_by` or `tail_position`) if the plan is handed to a subagent.
