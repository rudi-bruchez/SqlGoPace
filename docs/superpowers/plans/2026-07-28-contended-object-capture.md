...
- **Design spec:** `docs/superpowers/specs/2026-07-28-contended-object-capture-design.md` is the source of truth.
...
- Modify: `specs/SHRINK.md` (a short note that a failed shrink emits `.contended.yaml`, fed back via `plan --confirmed`)
...
- [ ] **Step 2: Update `specs/SHRINK.md`**

Add a short paragraph: on an `INCOMPLETE`/`FAILED` shrink, the engine writes `<manifest>.contended.yaml` listing the objects it held a `Sch-M` lock on while blocking others; a `.log` pointer line references it; `plan --confirmed <path>` feeds it into the next pre-shrink pass. Cross-reference `docs/superpowers/specs/2026-07-28-contended-object-capture-design.md`.

- [ ] **Step 3: Verify no stale references**

Run: `grep -rn "contended" README.md specs/SHRINK.md`
Expected: the new references present and consistent.

- [ ] **Step 4: Commit**

```bash
git add README.md specs/SHRINK.md
