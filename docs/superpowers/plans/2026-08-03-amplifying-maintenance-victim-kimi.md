...
Review of `docs/superpowers/plans/2026-08-03-amplifying-maintenance-victim.md` against the design spec `docs/superpowers/specs/2026-08-03-amplifying-maintenance-victim-design.md` and the current codebase.
...
The design spec example (`specs/2026-08-03-amplifying-maintenance-victim-design.md`, §3.3, lines 274-301) includes a `first_eligible` timestamp for each killed session. The Task 7 implementation renders only `killed_at`, and `AmplifierKillEvent` / `capturedAmplifier` do not carry the field.

| Plan location | Spec requirement |
|---|---|
| `internal/run/amplifier_capture.go` (Task 7) | `killed_at` only |
| `specs/2026-08-03-amplifying-maintenance-victim-design.md` §3.3 | `first_eligible` plus `killed_at` |
...
The spec example (`specs/...-design.md` §3.2) reads:
...
- Task 9 correctly scopes documentation to `README.md`, `specs/SPECS.md`, and the version bump. The `config.yaml` sample block in Task 8 Step 11 is clear and opt-in.
