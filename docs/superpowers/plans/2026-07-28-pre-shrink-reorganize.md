...
- **Never emit REBUILD** in the pre-shrink pass (a rebuild grows the file — `specs/SHRINK.md` §400); emit `reorganize_index` only.
- **`type: log`** skips reorganize and the advisory entirely (a log shrink reclaims VLFs, not page density).
- Design source of truth: `docs/superpowers/specs/2026-07-28-pre-shrink-reorganize-design.md`.
