...
> Implemented before `docs/specs/COMPRESSION-SCOPE.md`, which needs the planner able to mark an
...
- **A general `Operation.Satisfied()` predicate** (`docs/specs/FIXES.md` #28/#158). A pure defrag
...
Docs to rewrite: `README.md:73,92` (§7); `docs/specs/crash-resumable.md:4,252`, which **normatively
defines the flag and its default-off rationale**; `docs/specs/graceful-stop.md:21`;
`docs/specs/FIXES.md:28,115-117,137,158,417` (#6 is defined by the flag, #11 by `RunRecord.Skipped`);
`docs/specs/superpowers/plans/2026-07-03-maintenance-window.md:992`.
...
- **`RunRecord.Skipped` changes population, not shape.** `history.go:21` defines it as
  ...
  column, different meaning, no migration. `docs/specs/FIXES.md:137` (#11) is already open on this
